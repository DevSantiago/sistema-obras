...
curl -X PATCH "http://localhost:3000/api/v1/proyectos-base/1662a485-54df-4264-bfdf-c2c62befb053/centros-costo/055e53a2-36a0-41c2-bef8-4005ce73fb66/estado" \
  ...
  ## Crear solicitud de pago proveedor

curl -i -X POST "$BASE_URL/api/v1/solicitudes-pago" \
  -H "Content-Type: application/json" \
  -H "Cookie: session_token=$SESSION_TOKEN" \
  --data-raw '{
    ...
    "categoria_gasto": "ADMINISTRATIVO",
    "medio_pago": "EFECTIVO",
    "descripcion": "Pago de factura proveedor",
    "valor_bruto": 5000000,
    "valor_impuestos": 0,
    "valor_retenciones": 0,
    "valor_descuentos": 0
  ...
  ### Crear solicitud de pago trabajador (individual)
  curl -i -X POST "$BASE_URL/api/v1/solicitudes-pago" \
  -H "Content-Type: application/json" \
  -H "Cookie: session_token=$SESSION_TOKEN" \
  --data-raw '{
